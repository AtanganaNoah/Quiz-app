...
<div class="fade-in max-w-2xl mx-auto">
    <div class="mb-8">
        <a href="{{ route('quizzes.show', $quiz) }}" class="text-sm font-medium flex items-center gap-1 mb-4" style="color:var(--muted)">← Retour</a>
        <h1 class="text-3xl font-bold">Modifier le quiz</h1>
    </div>
        ...
        <form method="POST" action="{{ route('quizzes.update', $quiz) }}" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          ...
                          style="border-color:var(--border)">{{ old('description', $quiz->description) }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3 p-4 rounded-xl" style="background:#F8FAFC; border:1px solid var(--border)">
                ...
                <div>
                    <label for="is_published" class="text-sm font-semibold cursor-pointer">Publié</label>
                    <p class="text-xs mt-0.5" style="color:var(--muted)">Les étudiants peuvent accéder à ce quiz</p>
                </div>
            </div>

            <div class="flex gap-3 pt-2">
                <button type="submit" class="btn-primary flex-1 justify-center py-3">Mettre à jour</button>
